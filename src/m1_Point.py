...
class Point(object):
    ...
    def get_distance_from_start(self):
        dist_x = self.x - self.start_x
        dist_y = self.y - self.start_y
        distance = math.sqrt((dist_x)**2 + (dist_y)**2)
        return distance
    def get_distance_traveled(self):
        return self.distance
    def closer_to(self, p2, p3):
        dist2 = math.sqrt((p2.x-self.x)**2 + (p2.y-self.y)**2)
        dist3 = math.sqrt((p3.x-self.x)**2 + (p3.y-self.y)**2)
        if(dist2<dist3):
            return p2
        elif(dist2>dist3):
            return p3
    ...
    def halfway_to(self, p2):
        half_x = (p2.x+self.x)/2
        half_y = (p2.y+self.y)/2
